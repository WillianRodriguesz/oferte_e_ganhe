const express = require('express');
const router = express.Router();
const storeController = require('../controllers/storeController');
const authenticateToken = require('../middlewares/authMiddleware');

// Rota para servir a página de lojas
router.get('/lojas/view', authenticateToken, storeController.exibirPaginaLojas);

// Rota para obter todas as lojas
router.get('/lojas', authenticateToken, storeController.listarLojas);

// Rota para obter uma loja por ID
router.get('/lojas/:id', authenticateToken, storeController.obterLoja);

// Rota para cadastrar uma nova loja
router.post('/lojas', authenticateToken, storeController.criarLoja);

// Rota para atualizar uma loja pelo ID
router.put('/lojas/:id', authenticateToken, storeController.atualizarLojaInfo);

// Rota para excluir uma loja pelo ID
router.delete('/lojas/:id', authenticateToken, storeController.excluirLojaInfo);

module.exports = router;
